...
const _config = {"syslogPath": "./log/rest-listener.log",
							"logPath": "./log/" + this.ID + ".json",
							"columnSpec": {cols: [30, 40,15, 30, 50, 50], padding: 10, prefix:"SYSLOG: "},
							"ID":null,
							"color":"\x1b[36m"
							"logPayloads":false,
						    "payloadsLogPath": "./log/" + this.ID + "-payloads.json"};
var status = 'open';
...
function Logger(config) {
	...
	if(jsonComparer.compareJSON(config, default_config).length > 1)
	{
		this._config = config;
	}
...
Logger.prototype.log = function (msg, execPoint, status, error) {
	...
	if(this.columnizer)
	{
		logOutput = (this.color ? this.color : '') + this.columnizer.columnify({data: [datestamp, (this.logData.ID ? this.logData.ID : 'no ID'), status, execPoint, msg, (error ? error.message : '')]});
	...
	{
		logOutput = (this.color ? this.color : '') + datestamp + "\t" + (this.logData.ID ? this.logData.ID : 'no ID') + "\t" + status  + "\t" + execPoint + "\t" + msg + "\t" + (error ? error.message : '');
	...
	this.writeToFile(this._config.logPath, logOutput);
	//this.flushToFile();
...
Logger.prototype.syslog = function (msg, execPoint, status, error) {
   ...
   this.writeToFile(logString, this.config.syslogPath);
...
Logger.prototype.writeToFile = function (String output, String path) {
